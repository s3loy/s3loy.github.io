...
> 本节和学校计网授课风格不同，仅作为计网入门，对408并无参考意义
> 对于没有接触过的同学可能会感觉知识高密度输入，但请先别疑惑，细节和底层可以在以后的学习里面去理解，**不要半途而废**
> 内容逻辑性较强，慢慢理解
...
![Package](Computer-Network/Package.gif)
...
### TCP 协议 / TCP
**TCP** 是互联网的基石。它的任务极其艰巨：**在不可靠的 IP 层之上，构建一个可靠的传输通道**。为了实现“无差错、不丢失、不乱序”的承诺，TCP 被设计成了一个极其复杂的**有限状态机 (FSM)**
...
* **全双工**：允许通信双方同时进行双向传输
![full_duplex_half_duplex](Computer-Network/full_duplex_half_duplex.png)
